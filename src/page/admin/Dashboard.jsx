...
useEffect(() => {
  if (activeTab === 'support' && viewMode === 'list') {
    const token = localStorage.getItem('auth_token');
...
}, [activeTab, selectedStatus, selectedPriority, searchTerm, viewMode]);

// Replace the fetchSupportRequests function (around line 300)
const fetchSupportRequests = async () => {
  try {
    ...
    if (data.success) {
      ...
      console.error('Failed to fetch support requests:', data.error);
      setSupportRequests([]);
    }
  } catch (err) {
...
// Replace the fetchSupportStats function (around line 320)
const fetchSupportStats = async () => {
  try {
    ...
    if (data.success) {
      ...
      console.error('Failed to fetch support stats:', data.error);
    }
  } catch (err) {
 ...
 // Replace the fetchTicketDetails function (around line 340)
...
// Replace the handleSubmitReply function (around line 380)
const handleSubmitReply = async (ticketId) => {
  ...
  <select
    onChange={(e) => {
      e.stopPropagation();
      handleUpdateTicketStatus(ticket._id, e.target.value);
    }}
    value={ticket.status}
...
{/* Add refresh button next to filters */}
<div className="bg-white rounded-xl shadow-sm p-4 border border-gray-200">
  <div className="flex flex-wrap gap-4 justify-between items-center">
    ...
    <button
      ...
      className="flex items-center space-x-2 text-sm bg-blue-50 text-blue-600 px-3 py-2 rounded-lg hover:bg-blue-100"
    >
...
<div className="border-t border-gray-200 pt-6">
  ...
  <textarea
    ...
    placeholder="Type your reply here... (users will see this response)"
    rows="4"
  ...
  <div className="flex items-center justify-between mt-4">
    <div className="flex items-center space-x-4">
      <label className="flex items-center space-x-2">
        ...
        />
        <span className="text-sm text-gray-700">Mark as internal note (admin only - user won't see)</span>
      </label>
      ...
      >
        ...
        <option value="Thank you for reaching out. Our team will review your request and get back to you within 24 hours.">Acknowledge receipt</option>
        <option value="We have reviewed your request and it has been assigned to our support team for further investigation.">Assigned to team</option>
        <option value="We've resolved this issue. Please let us know if you need further assistance.">Resolved message</option>
      </select>
    </div>
